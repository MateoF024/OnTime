...
public final class AdminOps {
    ...
    private static JsonArray advancementsJson(MinecraftServer server) {
        ...
        try {
            // 1.20.1 has no AdvancementHolder: getAllAdvancements() hands out
            // Advancement itself, which answers getId() where the holder
            // answers id(). Same loop, one type and one method name apart.
            for (net.minecraft.advancements.Advancement advancement
                    : server.getAdvancements().getAllAdvancements()) {
                out.add(advancement.getId().toString());
            }
