...
public class PlayerPreferences {
    ...
    public static void save() {
        try {
            ...
            OnTime.LOGGER.error("Failed to save player preferences", e);
        }
    }
            ...
            if (root != null && root.has("timerVisibility")) {
                ...
                for (Map.Entry<String, JsonElement> entry : visibility.entrySet()) {
                    try {
                        ...
                        OnTime.LOGGER.warn("Invalid UUID in player preferences: {}", entry.getKey());
                    }
                }
            }
        } catch (IOException e) {
            OnTime.LOGGER.error("Failed to load player preferences", e);
        }
    }
}